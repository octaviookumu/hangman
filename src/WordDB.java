...
public class WordDB {
    ...
    public String[] loadChallenge() {
        ...
        // rand.nextInt(categories.size()) generates a random integer between 0 (inclusive) and categories.size() (exclusive)
        // categories.get(...) retrieves the element at the randomly generated index
        String category = categories.get(rand.nextInt(categories.size()));

        // categoryValues holds all the words in the chosen category
        ...
        // rand.nextInt(categoryValues.length) generates a random integer from 0 (inclusive) to categoryValues.length (exclusive),
        // which serves as a random index within categoryValues


        // [0] -> category and [1] -> word
        return new String[]{category.toUpperCase(), word.toUpperCase()};
        // new String[]{...} creates a new array of String objects
        // The array contains two elements: the uppercase version of category as the first element
        // and the uppercase version of word as the second element
